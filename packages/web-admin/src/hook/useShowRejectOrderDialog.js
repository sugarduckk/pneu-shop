...
const RejectOrderDialog = ({ orderId }) => {
  const { user } = useGlobalState()
  const dismiss = useDismissDialog()
  const updateDialog = useUpdateDialog()
  const rejectOrder = useRejectOrder(user.uid, orderId)
...
const useShowRejectOrderDialog = (orderId) => {
  const addDialog = useAddDialog()
  return React.useCallback(() => {
    addDialog(<RejectOrderDialog orderId={orderId} />)
  }, [addDialog, orderId])
}
